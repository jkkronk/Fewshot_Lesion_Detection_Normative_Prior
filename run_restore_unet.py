...
from restoration import MAP_unet
...
if __name__ == "__main__":
    ...
    parser.add_argument('--vae_name', type=str, default=0)
    parser.add_argument('--unet_name', type=str, default=0)

    opt = parser.parse_args()
    name = opt.name
    fprate = opt.fprate
    vae_name = opt.vae_name
    unet_name = opt.unet_name
    ...
    print(' Vae model: ', vae_name,' Unet model: ', unet_name, ' Fprate: ',fprate)
    ...
    path = unet_name + '.pth'
    unet = torch.load(path, map_location=torch.device(device))
    unet.eval()
    ...
    for subj in subj_list: # Iterate every subject
        ...
        for batch_idx, (scan, seg, mask) in enumerate(subj_loader):
            ...
            restored_batch = MAP_unet(scan, decoded_mu, vae_model, unet, riter, device, weight, step_rate, writer)
